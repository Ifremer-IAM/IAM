    ...
    if ((delay<=ind_t) & (gestInd==1) & (DELAY>0)) { //DELAY = 1 -> on remet l'effort au niveau de l'instant initial
      ...
      if (N_SPPspictOPT>0) { //si esp�ce dynamique SPICT
        ...
        for (int i = 0; i < N_SPPspictOPT; i++){
          ...
          for (int ag = 0; ag < nbi; ag++) Fothi2[ag + ind_t*nbi] = Fothi2[ag + (DELAY-1)*nbi];
...
extern "C" {
  ...
  void BioEcoPar::GestionF2(int ind_t, int VERBOSE)
  {
    if(VERBOSE){Rprintf("\n   Gest: ");}
    ...
    for (int indF = 0 ; indF < nbF ; indF++) {
      ...
      Rprintf("%4.2f %4.2f ; ",g_effSup[indF + nbF*ind_t], denom_st1);
      if (denom_st1>0) alpha_f_st1 = g_effSup[indF + nbF*ind_t] / denom_st1;
      Rprintf("%4.2f | ",alpha_f_st1);
      for (int indM = 0 ; indM<nbMe ; indM++) {
    ...
    for (int ind_f = 0 ; ind_f < nbF ; ind_f++) {
      for (int ind_m = 0 ; ind_m<nbMe ; ind_m++) {
        Rprintf("%4.2f ", EffsupTMP_fm[1 + ind_f + nbF*ind_m]);
      }
      Rprintf("\n");
    }
    ...
    for (int ind_f = 0 ; ind_f<nbF ; ind_f++)
      ...
      if (N_SPPdynOPT>0) {
        ...
        Rprintf("\n");
        // Rf_PrintValue(getListElement(getListElement(listTempP, "Fleet"), "effort1_f_m"));
        // Rf_PrintValue(getListElement(getListElement(listTempP, "Fleet"), "effort2_f_m"));
        // Rf_PrintValue(getListElement(getListElement(listTempP, "Fleet"), "nbTrip_f_m"));
        //fichier << "ST11" << endl;
