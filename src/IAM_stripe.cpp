...
#include "BioEcoPar_stripe.h" // Class is defined in this file.
...
// -std=gnu++11 -shared -s -static-libgcc -o

// [[Rcpp::export]]
List IAM_stripe(List listInput, List listSpec, List listStochastic, List listScen,
                IntegerVector RecType1, IntegerVector RecType2, IntegerVector RecType3,  
                IntegerVector Scenarii,  IntegerVector Bootstrp,  IntegerVector nbBoot,
                IntegerVector GestInd, NumericVector mOth, NumericVector bounds, 
                List TAC, 
                NumericVector FBAR,  List othSpSup,  NumericVector effSup,  IntegerVector GestParam,  NumericVector EcoDcf,
                NumericVector persCalc, NumericVector dr, 
                IntegerVector SRind, List listSR, List TypeSR, 
                NumericVector mFM,  
                List TACbyF, List Ftarg,  List W_Ftarg, List MeanRec_Ftarg,
                List parBHV, List parQEX, List tacCTRL, List stochPrice, 
                IntegerVector updateE, List parOQD, 
                CharacterVector bootVar = R_NilValue, IntegerVector verbose = 0
){
int VERBOSE = verbose[0];
if(VERBOSE){Rprintf("This is a start ! :) \n");}
    if (Bootstrp[0]==0) {
        if(VERBOSE){Rprintf("No bootstrap \n");}
        BioEcoPar_stripe *object = new BioEcoPar_stripe(listInput, listSpec, listStochastic, listScen,
...
//     }
    ...
    CharacterVector v = {"This","is","neat"};
    List L = List::create(Named("Hello Rcpp") = v);
    return(L);
}
