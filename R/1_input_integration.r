...
if (is.null(folderFleet)) {
  ...
  namList <- sapply(nam_stock,function(x) gsub("Stock__","",x)) # extract species names
#if (is.null(folderFleet)) {
...
listInput[is.na(names(listInput))] <- NULL
      ...
      LL$historique[[length(nam_stock)+k]] <- listHisto
      LL$input[[length(nam_stock)+k]] <- listInput
      LL$scenario <- c(LL$scenario,listScenar)
