...
#on commence par s'occuper du slot 'specific', dans lequel sont renseign?es les dimensions
...
#on a alors toute l'info n?cessaire pour proc?der aux formatage des objets dans les autres slots
...
unl <- function(ll,file="input.txt",spec=list(),flag=NA) {
  if (is.list(ll)) {  #si c'est une liste...
    ...
    dimMet <- dimAC <- "NA"
    if (!is.null(dimcst)) {
      if (dimcst[3]>0) { #il faut alors distinguer les ?ges des cat?gories
        if (identical(dimnames(ll)[[sum(dimcst[1:2]>0)+1]],spec$Ages[[flag]])) dimAC <- "A" else dimAC <- "C"
      }
      if (dimcst[2]>0) { #il faut alors distinguer les m?tiers bio des m?tiers ?co
    ...
    #doivent ?tre quand m?me format?es
    #pas besoin de formatage pouss?, juste la mise en forme suffit
      if (is.matrix(ll)) {
...
#'
#' @param object TODO
#' @param folder The folder in which the output .txt files will be exported. Chr.
...
#' @rdname IAM.export
#' @export
setGeneric("IAM.export", function(object, folder, ...){ # IAM.export meth ####
...
#' @rdname IAM.export
#' @aliases IAM.export,iamInput-method
setMethod("IAM.export", signature("iamInput"), function(object, folder, ...){ ## iamInput meth ####
...
#' @rdname IAM.export
#' @aliases IAM.export,iamArgs-method
setMethod("IAM.export", signature("iamArgs"), function(object, folder, ...){ ## iamArgs meth ####
