...
```{r example_data, include=FALSE, echo = FALSE}
...
fileIN <- IAM_example("inputFile.xlsx") # simplified with only 2 dyn and 2 static
load("../dev/data/inpSS3darwiniana1984.RData")
  input1984 <- IAM::IAM.input(fileIN = fileIN,t_init=1984,nbStep=12,folderFleet="../dev/data/fleets",
...
L'objet résultant (ici `input1984`) de classe `iamInput` possède une structure fixe spécifique, composée d'éléments appelés "slots" et que l'on peut distinguer de cette façon :
...
```{r inpnames}
names(input1984@specific)
names(input1984@input)
names(input1984@input$COR)
input1984@input$COR$wStock_i
```
...
```{r argum, eval=F}
argum1984 <- IAM.args(inp)
```

```{r falseargum, include = FALSE, echo = FALSE, results="hide"}
