...
Une interface reprenant tous les arguments auparavant validés s'ouvre, permettant la correction, la validation, et ainsi la mise à jour de l'objet `IAM_argum_1984` en sortie.
...
```{r simul}
IAM_out_1984 <- IAM.model(IAM_argum_1984,IAM_input_1984)
class(IAM_out_1984)
```

Toutes les sorties du modèle sont entreposées dans 2 slots de l'objet `IAM_out_1984`. Un slot `outputSp` réunit toutes les variables définies par espèce modélisée (sorties du modèle biologique principalement), et un autre slot `output` rassemble les variables sans dimension espèce (les variables économiques). L'accès aux variables est illustré ci-dessous :

```{r slotsim}
slotNames(IAM_out_1984)
names(IAM_out_1984@outputSp)
names(IAM_out_1984@output)
```
...
```{r simrep}
# Activate replicates
IAM_argumRep_1984 <- IAM.editArgs_Rep(IAM_argum_1984, n = 2)

IAM_outRep_1984 <- IAM.model(IAM_argumRep_1984,IAM_input_1984)
class(IAM_outRep_1984)
length(IAM_outRep_1984@outputSp$F)
names(IAM_outRep_1984@outputSp$F[[1]])
names(IAM_outRep_1984@outputSp$F[[2]])
```
<!-- </details>  -->
...
statu_quo_tbl_Fbar <- IAM.format(IAM_out_1984, name = "Fbar", 
                                 sim_name = "statu_quo", n = 1)
head(statu_quo_tbl_Fbar)
# Extraction de plusieurs variables
statu_quo_tbl_summary <- IAM.format(IAM_out_1984, name = "summary", 
                                    sim_name = "statu_quo", n = 1)
...
```{r iam.format_quant}
res1 <- IAM.format(IAM_out_1984, c("SSB"), n = 1) %>%
 dplyr::filter(species == "ARC", year <= 1990)
...
```{r simu2plot}
N = 5
IAM_argum_1984@arguments$Recruitment$COR$wnNOISEmodSR <- 5e6
# Simulation
...
TACvsSQ_5simuls <- do.call(rbind, c(Statuquo_l, TACglobv_nbt_l, TACglobv_nbv_l)) %>%
...
```{r plotdiffscenar, fig.cap=paste("\\label{fig:figs}Fbar evolution for COR species",N, "runs." )}
TACvsSQ_5simuls %>%
...
clusterExport(cl,c("IAM_argum_1984", "IAM_input_1984"))
