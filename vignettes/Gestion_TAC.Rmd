...
```{r plot_bio, fig.cap=paste("\\label{fig:TACglob}COR variables under global TAC gestion.",N, "runs." )}
TACglob_quant %>%
...
```{r TACglob_plot, fig.cap=paste("\\label{fig:TACglob}Effort variables under global TAC gestion.",N, "runs." )}
TACglob_quant %>%
...
TACglob_quantR <- do.call(rbind, c(TACglob_nbvR)) %>%
  ...
  mutate(sim_name = factor(sim_name, levels = c( # Reorder the column in plot
    ...
    "TAC global nbv", "Report TAC global nbds")))
```

La comparaison montre une dimension du Fbar sur l'espèce cible COR.

```{r TACreport_bio, fig.cap=paste("\\label{fig:TACglob}COR variables under global TAC gestion with effort report from Filet_COR to Filet_DP.",N, "runs." )}
TACglob_quantR %>%
  filter(species == "COR", variable %in% c("Fbar", "SSB", "L_et")) %>%
  ggplot(aes(x = year, y = median)) + 
  facet_grid(variable ~ sim_name, scales = "free_y") +
