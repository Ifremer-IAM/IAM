...
Cette simulation permet de diminuer drastiquement la mortalite par pêche de l'espèce dite "COR" avec une reduction immédiate la taille des flottilles. On note d'ailleurs que seules 2 flottilles comportent encore plus de 2 navires à la fin de la simulation, ce qui est catastrophique.

```{r TAC}
sim_TACglob <- IAM::IAM.model(objArgs = IAM_argum_1984_TAC, objInput = IAM_input_1984)

tbl_TACglob <-  IAM.format(sim_TACglob, "summary", sim_name = "TAC global", n = 1)
tbl_TACglob %>% 
  filter(variable == "nbv_f") %>% 
  ggplot(aes(x = year, y = value, color = fleet)) + geom_line() +
