...
sp2rm <- enigmaSp$origin[enigmaSp$code %in% c("DAR","GAR")]
...
if(tea_br){
  ...
  for(s in sp2rm){
    removeWorksheet(wb, paste0("Stock__", s))
    save_names <- save_names[!grepl(s, save_names)]
  }
  ...
  ## Copy files as new sheets ####
  for(i in 1:nrow(enigmaF)){
    cat(" -",enigmaF$code[i])
    fnme <- enigmaF$code[i]
    nme <- paste0("dev/data/fleets/",fnme, ".csv")
    sheet <- read.csv(nme, sep = ";")
    addWorksheet(wb, sheet = fnme)
    writeData(wb, sheet = fnme, fnme, colNames = FALSE, rowNames = FALSE)
    save_names <- c(save_names, fnme) # uggly but no choice here
  }
